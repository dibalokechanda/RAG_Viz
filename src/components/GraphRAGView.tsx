...
const STAGES: Stage[] = [
  ...
  },
  {
    ...
    caption: 'A hypothetical answer seeds a lightweight global search; every follow-up question it raises then gets its own local search, twice.',
    panel: [
      '**Dynamic Reasoning and Inference with Flexible Traversal.** Microsoft’s third search mode, and the one that stops local and global being a either/or choice. It runs a global-flavoured primer first, then local searches over the questions that primer raises.',
      '**Step 1, HyDE.** The query is not embedded directly. The model first writes a *hypothetical* answer, the kind of passage the corpus would already contain, and **that** is embedded. A written answer sits closer in vector space to real corpus text than a bare question does, so the retrieval is sharper.',
      '**Step 2, the primer.** The top-k community reports nearest that vector produce an initial answer plus a list of follow-up questions. This is global search in miniature, and it costs a fraction of a full map-reduce.',
      '**Step 3, the refinement loop.** Each follow-up gets its own local search, and each of those emits an intermediate answer and new follow-ups. Crucially these local searches are informed by **both** community-level knowledge and entity-level detail, which is what lets DRIFT stay useful when the query diverges from the persona the index was built for.',
      '**Step 4, reduce.** The result is a hierarchy of question and answer nodes ranked by relevance to the original query. Map-reduce again, weighting every intermediate answer equally, then one synthesis call.',
      '**Two iterations is hard-coded.** Microsoft note that a learned reward function for smarter termination is future work, so today the loop depth is a constant rather than something the query decides.',
    ],
    cards: [
