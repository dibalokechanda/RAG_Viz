...
export LANGCHAIN_PROJECT=rag-prod`,
...

  {
    ...
    detail: [
      ...
      '- **A bad promotion** is fixed by rolling the alias back to the previous manifest (seconds, not a rebuild, which is why artifacts are versioned as a unit).',
      '- **A component failing mid-request** needs the request itself to degrade rather than fail.',
      '**Every optional stage is a fallback level:**',
      '- If the reranker times out, serve the retrieval order (worse, but valid).',
      '- If generation fails, return the retrieved passages with citations rather than nothing.',
      '- If retrieval returns nothing above the similarity floor, say the corpus does not cover it (a correct answer, and far better than inventing one).',
      '**Make degradation deliberate and visible:** A silent fallback that quietly serves worse answers is indistinguishable from a quality regression, and will be diagnosed as one for a long time. Record the degradation level on every response and alert on its rate, not on individual occurrences.',
    ],
    math: [
      {
    ...
    concepts: [
      {
        ...
        detail: [
          '**The problem:** When a component (like a reranker) is failing, sending it every request and waiting for each to time out turns a degraded component into a latency outage across the whole system.',
          '**The solution:** Trip a circuit breaker after a failure threshold and skip that stage entirely for a cool-down window (serving the fallback level directly).',
          '- Probe occasionally to decide when to close it again.',
          '- The pipeline shape makes this unusually clean, because the stage was optional to begin with.',
        ],
      },
      {
        id: 'fb-timeout',
      ...
      },
      {
        ...
        detail: [
          '**When it helps:** If the index is unavailable, a cached answer to the same normalised query is usually preferable to an error (for informational queries).',
          '**When it hurts:** For anything time-sensitive or policy-bearing it is not, because a stale answer to "what is the current policy" is wrong in a way an error is not.',
          '**The solution:** Make staleness tolerance a property of the query class established back at query understanding, not a global setting.',
        ],
      },
    ],
  },
