...
export const onlineStages: Stage[] = [
...

  {
    ...
    concepts: [
      {
        id: 'rr-late',
        label: 'Late interaction — the middle ground',
        kind: 'method',
        summary: 'ColBERT: per-token vectors, cheap interaction',
        detail: [
          'Bi-encoders compress a document to one vector before the query arrives; cross-encoders defer everything until it does. Late interaction sits between: store one vector per token, then score with a cheap operator at query time.',
          'It recovers much of the cross-encoder’s accuracy while keeping documents precomputable — at the cost of storing hundreds of vectors per chunk instead of one, which is a serious index-size multiplier.',
        ],
        math: [
          {
            title: 'MaxSim',
            tex: String.raw`s(q, d) = \sum_{i \in q} \max_{j \in d} \left( e_{q_i} \cdot e_{d_j} \right)`,
            note: 'Each query token finds its best match anywhere in the document, and those maxima are summed. No shared attention, so document vectors stay precomputable.',
          },
        ],
      },
      {
