    ...
    concepts: [
      ...
      },
      {
        ...
        detail: [
          ...
          'The right way to see the gain is recall@K after reranking a top-N pool, versus recall@K straight from retrieval. Typical improvements are 10–20 points of nDCG@K, which is larger than the gain from most embedding-model upgrades.',
        ],
      },
      {
        id: 'rr-skip',
        ...
        detail: [
          'If retrieval precision@K is already high (e.g., a small, clean, well-separated corpus), reranking adds latency for little movement.',
    ...
    concepts: [
      ...
      },
      {
        ...
        detail: [
          ...
          'Measure it at the retrieval stage with a generous K (e.g., K=50). If recall@K is low, nothing downstream can save you and the fix belongs in chunking, embedding, or query processing.',
        ],
        math: [
          {
            title: 'Definition',
        ...
        children: [
          {
            ...
            detail: [
              'Recall is determined at retrieval and can only fall afterwards. Measuring recall@K on the post-rerank set conflates two things: whether retrieval found the chunk, and whether the reranker kept it.',
              'Track recall at a large K (e.g., K=50) during retrieval and precision at a smaller K (e.g., K=5) after reranking. Those two numbers localise a failure to a stage; a single end-to-end number does not.',
            ],
          },
        ],
      },
...
chain = create_retrieval_chain(retriever,
    ...
    detail: [
      ...
      '**Ordering and labelling are not cosmetic:**',
      '- **Clearly delimited chunks** with stable identifiers are what make citation possible downstream.',
      '- **Position within the context window** measurably affects what the model attends to.',
    ],
    stack: [
    ...
    concepts: [
      {
        ...
        detail: [
          ...
          '- **Keep k small** and order deliberately (put the highest-ranked chunks at the very start and the very end).',
          '- **Avoid "just use the big context window"**—a 100-chunk prompt buries most of its own evidence.',
          '**This is the single strongest argument for taking reranking seriously.** If only the first and last few positions are read reliably, then which chunks occupy them determines the answer. That decision is made by the reranker, not by retrieval.',
        ],
        figures: [
          {
            kind: 'curve',
      ...
      },
      {
        ...
        detail: [
          ...
          '**Keep the identifier short** (e.g., [1], [2]): every token spent on scaffolding is a token not spent on evidence, and long identifiers get transcribed incorrectly.',
        ],
      },
      {
        id: 'pr-abstain',
        ...
        detail: [
          'Without an explicit instruction, a model handed weak context will still produce a confident answer; that is what its training rewards.',
          '**The highest-value line in most RAG system prompts:** permission to say the context does not contain the answer.',
      ...
      },
      {
        ...
        detail: [
          '**Descending relevance:** The obvious default. Puts the best chunk where attention is strongest.',
          '**Ascending order (best chunk last):** Exploits recency and often measures slightly better, since the strongest evidence is adjacent to the instruction.',
          '**Interleaving:** Alternates from both ends inward, so the top two chunks occupy the first and last positions.',
          'Whichever you choose, fix it and measure it; this is a cheap A/B with a real effect size.',
        ],
      },
    ],
